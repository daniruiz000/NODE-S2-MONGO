...
const { resetCars } = require("../utils/restetCars.js");
...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    const car = await Car.findById(id); //  Buscamos un documentos con un id determinado dentro de nuestro modelo con modelo.findById(id a buscar).
    if (car) {
...
router.get("/brand/:brand", async (req, res) => {
  ...
  try {
    ...
    const car = await Car.find({ brand: new RegExp("^" + brand.toLowerCase(), "i") }); //  Esperamos a que realice una busqueda en la que coincida el texto pasado por query params para la propiedad determinada pasada dentro de un objeto, porqué tenemos que pasar un objeto, sin importar mayusc o minusc.
    if (car?.length) {
